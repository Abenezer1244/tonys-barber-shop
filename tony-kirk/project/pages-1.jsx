...
function TeamPage({ navigate }) {
  return (
    <div className="page-fade">
      <section className="page-hero">
        <div className="container">
          <p className="eyebrow eyebrow-cream">— The team</p>
          <h1 className="display h1" style={{ color: "var(--bg)", marginTop: 12 }}>Four barbers.<br/>One standard.</h1>
          <p className="lede">Every barber in the shop has at least two years in the chair. Tony reviews every cut before you walk out. Pick your barber below — or just book "any available" and we'll match you.</p>
        </div>
      </section>
          ...
          <div className="team-grid">
            {TEAM.map((m) => (
              <article className="team-card" key={m.name}>
                <div className="team-card-img" style={{ backgroundImage: `url('${m.img}')` }} />
                <div className="team-card-body">
                  <p className="team-card-role">{m.role} · {m.years}</p>
                  <h3 className="team-card-name">{m.name}</h3>
                  <p className="team-card-bio">{m.bio}</p>
                  <div className="team-specialties">
                    {m.specialties.map((s) => <span key={s} className="spec-tag">{s}</span>)}
                  </div>
                  <div className="team-card-foot">
                    <button className="btn btn-ghost-dark btn-sm" onClick={() => navigate("book")} style={{ width: "100%" }}>
                      Book with {m.name} <Icon name="arrow" size={14} />
                    </button>
                  </div>
                </div>
              </article>
      ...
      <section className="section-pad-sm" style={{ background: "var(--bg-cream)" }}>
        <div className="container" style={{ textAlign: "center" }}>
          ...
          <h2 className="display h2" style={{ marginTop: 8 }}>Want a chair?</h2>
          <p className="lede" style={{ margin: "16px auto 24px" }}>We're always open to talking with experienced barbers who want to do good work in a calm shop.</p>
          <button className="btn btn-primary btn-lg" onClick={() => navigate("careers")}>See Open Roles</button>
        </div>
      </section>
    </div>
  );
...
Object.assign(window, { ServicesPage, AboutPage, TeamPage });
