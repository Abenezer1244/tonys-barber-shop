    ...
    <div className="page-fade">
      ...
      <section className="hero">
        ...
        <div className="hero-content container">
          <div className="hero-meta-row" style={{ marginBottom: 24 }}>
            ...
            <span className="dot">●</span>
            <span>★ {window.SHOP.rating} ({window.SHOP.reviewCount} Google reviews)</span>
          </div>
          <div className="hero-grid">
            <div>
              <h1 className="hero-tagline">Where men get<br/><span className="accent">ready.</span></h1>
              <p className="hero-sub">Mid-range master barbering in {window.SHOP.neighborhood}, {window.SHOP.city}. Sharp cuts, hot-towel shaves, and a chair you'll want to keep coming back to. Walk in or book ahead — everyone welcome.</p>
              <div className="hero-cta-row">
                <button className="btn btn-primary btn-lg" onClick={() => navigate("book")}>Book Now <window.Icon name="arrow" size={14} /></button>
            ...
            <div className="hero-side-card">
              ...
              <p style={{ color: "rgba(244,237,227,0.7)", fontSize: 13, margin: "0 0 8px" }}>Or call <a href={`tel:${window.SHOP.phoneRaw}`} style={{ color: "var(--brass)" }}>{window.SHOP.phone}</a>.</p>
              <div className="book-card">
                ...
                <button className="btn btn-primary btn-lg" style={{ marginTop: 4 }} onClick={() => navigate("book")}>
                  Confirm Appointment <window.Icon name="arrow" size={14} />
                </button>
              </div>
            </div>
          </div>
      ...
      <section className="value-strip">
        <div className="value-grid">
          {window.VALUE_PROPS.map((v) => (
            <div className="value-item" key={v.title}>
              <span className="value-icon"><window.Icon name={v.icon} size={18} /></span>
      ...
      <section className="section-pad">
        <div className="container">
          ...
          <div style={{ marginTop: 40, textAlign: "center" }}>
            <button className="btn btn-ghost-dark btn-lg" onClick={() => navigate("services")}>See Full Menu <window.Icon name="arrow" size={14} /></button>
          </div>
        </div>
      </section>
          ...
          <div className="split">
            ...
            <div>
              ...
              <div style={{ marginTop: 32 }}>
                <button className="btn btn-primary" onClick={() => navigate("about")}>Read Our Story <window.Icon name="arrow" size={14} /></button>
              </div>
            </div>
          </div>
        </div>
          ...
          <div className="reviews-head">
            <div>
              ...
              <div className="rating-big" style={{ marginTop: 12 }}>
                <span className="num">{window.SHOP.rating}</span>
                <div>
                  <window.Stars n={5} />
                  <div className="meta" style={{ marginTop: 4 }}>{window.SHOP.reviewCount} verified reviews</div>
                </div>
              </div>
            </div>
            <a className="btn btn-ghost-dark"><window.Icon name="google" size={14} /> Read all on Google</a>
          </div>
          <div className="review-grid">
            {window.REVIEWS.slice(0, 6).map((r, i) => (
              <article className="review-card" key={i}>
                <window.Stars n={r.stars} />
        ...
        <div className="container">
          <div className="map-grid">
            ...
            <div className="contact-info">
              ...
              <h3 className="display h3" style={{ color: "var(--bg)", margin: "8px 0 8px" }}>{window.SHOP.city}, {window.SHOP.neighborhood}</h3>
